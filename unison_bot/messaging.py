...
async def game_one(message: types.Message, state: FSMContext):
  await state.reset_state(with_data=False)
  if await db.is_matching(message.from_user.id, conn):#data['has_match']:
    await Form.has_match.set()
  else:
    await Form.no_match()
...
async def game_one(message: types.Message, state: FSMContext):
  await state.reset_state(with_data=False)
  if db.is_matching(message.from_user.id, conn):#data['has_match']:
    await Form.has_match.set()
  else:
    await Form.no_match()
...
async def game_one(message: types.Message, state: FSMContext):
  await state.reset_state(with_data=False)
  if db.is_matching(message.from_user.id, conn):
    await Form.has_match.set()
  else:
    await Form.no_match()
...
async def get_help(message: types.Message, state: FSMContext):
  ...
  #data['help']=True
  await db.set_help_status(message.from_user.id, conn)
  await state.reset_state(with_data=False)
...
# })
  ...
  await bot.send_message(message.from_user.id, text=texts.GET_HELP)
  await Form.help.set()
  #-------------------------------------------------------------------------
